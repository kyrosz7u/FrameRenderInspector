...
private:
	...
	FString SearchText;
	EInspectorMode ActiveMode = EInspectorMode::All;
	...
	void ApplyFilteredTextureSelection();
	void ApplyFilteredBufferSelection();
	void SetInspectorMode(EInspectorMode NewMode);
	FReply OnModeButtonClicked(EInspectorMode NewMode);
	FSlateColor GetModeButtonColor(EInspectorMode Mode) const;
	bool MatchesActiveMode(const FString& ResourceName) const;
